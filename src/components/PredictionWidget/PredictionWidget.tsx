...
const PredictionWidget: React.FC = () => {
  const { fetchKlineData, prices, watchlist } = usePriceContext();
  const botCoins = useMemo(
    () => watchlist.filter((w) => w.autoTradeEnabled),
    [watchlist]
  );
  ...
  useEffect(() => {
    ...
    runPredictions();
    const interval = setInterval(runPredictions, activeTimeframe === '1m' ? 30000 : 60000);

    return () => {
      mounted = false;
      clearInterval(interval);
    };
  }, [fetchKlineData, activeTimeframe, botCoins]);
      ...
      <div className={styles.list}>
        ...
        {botCoins.length === 0 ? (
          <div className={styles.emptyState}>
            No BOT-enabled symbols. Enable BOT on the Watchlist to scan.
          </div>
        ) : (
          botCoins.map((coin) => {
